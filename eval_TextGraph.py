...
def inference(detector, test_loader, output_dir):
    ...
    for i, (image, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, kernel, border,meta) in enumerate(test_loader):

        image, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, kernel, border = to_device(
            image, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, kernel, border)
        ...
        contours, output = detector.detect1(image)
        tr_pred, tcl_pred , kernel_pred, border_pred = output['tr'], output['tcl'], output['kernel'], output['border']
        ...
        pred_vis = visualize_detection(img_show, contours, tr_pred, tcl_pred, kernel_pred, border_pred)
        ...
        gt_vis = visualize_gt(img_show, gt_contour,
                              tr_mask[idx].cpu().numpy(), tcl_mask[idx, :, :, 0].cpu().numpy(),kernel[idx].cpu().numpy(),border[idx].cpu().numpy())
...
def main(vis_dir_path):
    ...
    osmkdir(vis_dir_path)   #./vis/Ctw1500
